...
function identityFromClaims(account:AccountInfo):AdminIdentity{const claims=account.idTokenClaims as Record<string,unknown>|undefined;const tokenRoles=Array.isArray(claims?.roles)?claims.roles.filter((role):role is string=>typeof role==='string'):[];const isAdmin=tokenRoles.includes('Admin')&&account.username.toLowerCase()==='sivmm29@gmail.com';const expires=typeof claims?.exp==='number'?new Date(claims.exp*1000).toISOString():undefined;return{id:account.localAccountId,name:account.name||'BetterSnap Super Admin',email:account.username,roles:isAdmin?['SuperAdmin']:tokenRoles,permissions:isAdmin?[...permissionsForRoles(['SuperAdmin'])]:[],sessionExpiresAt:expires}}
async function toIdentity(account:AccountInfo):Promise<AdminIdentity>{const claimsIdentity=identityFromClaims(account);try{const server=(await adminService.getMe()).data;const isAdmin=server.roles.includes('Admin')&&server.email.toLowerCase()==='sivmm29@gmail.com';return{...claimsIdentity,id:server.oid,name:server.name||claimsIdentity.name,email:server.email,roles:isAdmin?['SuperAdmin']:server.roles,permissions:isAdmin?[...permissionsForRoles(['SuperAdmin'])]:[]}}catch(error){if(error instanceof ApiError&&error.status===404)return claimsIdentity;throw error}}
...
export function AuthProvider({children}:{children:ReactNode}){
  ...
  const login=useCallback(async()=>{const account=await entraSignIn();const next=await toIdentity(account);if(!next.permissions.length)throw new Error('Access is restricted to sivmm29@gmail.com with the BetterSnap Admin role.');setIdentity(next)},[])
