...
export function DashboardPage(){
  ...
  const metrics=[
    ...
    ['Active users (30d)',dashboard.users.active30d.toLocaleString(),<Users size={16}/>],
    ['Suspended users',value(dashboard.users.suspended),<Users size={16}/>],
    ['Paying users',dashboard.users.paying.toLocaleString(),<Coins size={16}/>],
    ['Active subscriptions',value(dashboard.billing.activeSubscriptions),<Coins size={16}/>],
    ...
    ['Credits purchased (30d)',dashboard.billing.creditsPurchased30d.toLocaleString(),<Coins size={16}/>],
    ['Credits used',value(dashboard.billing.creditsUsed),<Coins size={16}/>],
  ] as const

  return <div className="dashboard-view">
    <PageHeader title="Super Admin Dashboard" description="Live server-side aggregates, recent audit activity, and platform health." action={<div className="header-meta-actions"><span className="last-updated">Last updated: {dashboard.updatedAt.toLocaleString()}</span><button className="button secondary" onClick={state.reload}><RefreshCw size={14} className={state.loading?'spin':''}/>Refresh</button></div>}/>
    <div className="notice-banner"><div className="notice-content"><strong>Server-side dashboard summary</strong><p>Dashboard metrics come from <code>/api/superadmin/dashboard/summary</code>. Fields not supplied by the endpoint are shown as “Not returned.”</p></div></div>
    ...
    <section className="dashboard-section"><div className="section-title-wrap"><div><h2>Live platform metrics</h2><p>Aggregated by the backend without downloading complete paginated datasets.</p></div><DataSourceBadge type="live"/></div><div className="metrics-grid">{metrics.map(([label,metric,icon])=><article className="metric-card" key={label}><div className="metric-top"><span>{label}</span><span className="metric-icon">{icon}</span></div><strong>{metric}</strong></article>)}</div></section>
