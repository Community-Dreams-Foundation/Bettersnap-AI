...
def reserve_job_slot(user_id, input_blob_path, job_params,
    ...
    try:
        ...
        # Teams membership wins over every personal balance. Resolve it INSIDE the app
        # lock so membership, sufficiency, job attribution, and debit are one transaction.
        ...
        if not org_id and source_type in ("monthly", "one_time"):
            try:
        ...
        # organization_id durably records which pool paid so refunds remain correct
        # even if the member later leaves the organization. source_type continues to
        # identify the requested product for retention and LoRA lifecycle behavior.
        cur.execute("""
            INSERT INTO jobs
                (user_id, status, input_blob_path, job_params, organization_id, source_type)
            OUTPUT INSERTED.job_id
            VALUES (?, ?, ?, ?, ?, ?)
        """, user_id, initial_status, input_blob_path, job_params, org_id, source_type)
