...
class BodyGuardTests(unittest.TestCase):
    ...
    def _assert_object_400(self, body_bytes):
        ...
        self.assertEqual(json.loads(resp.body)["error"], "Request body must be a JSON object")
...
class ImageCountValidationTests(unittest.TestCase):
    ...
    PLAN_ROW = ("monthly_pro", "ready", 200)   # plan_name, lora_status, credits_remaining
    ...
    def setUp(self):
        self.reserve = mock.Mock(return_value=types.SimpleNamespace(
            ok=True, job_id="job-123", outbox_id=1, reason=None))
        patches = [
